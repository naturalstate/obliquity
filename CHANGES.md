...
### 26. Renamed the login pillar `hydra` -> `brute`; hydra credentials in reports; response filters (2026-09-24)

- **Pillar rename `hydra` -> `brute`** (your call): the pillar is now named for
  its *function* like `bust`/`fuzz`/`crack`, with `hydra` demoted to the
  backend tool (medusa/ncrack become future backends under `brute`, mirroring
  hashcat+john under `crack`). All commands moved: `brute job add/list/remove`,
  `brute plan/run/resume`, `brute creds`. DB tables were already
  function-named (`login_*`), so no data migration; `adapters/hydra.py`,
  `core/loginplan.py`, `core/login_runner.py`, and `login_profiles/` keep their
  neutral names. `gameplans list brute`, `history --tool hydra`, `doctor`, and
  `TOOL_LABELS` (hydra -> "brute") all updated.
- **Per-project default extended to `brute`**: new `default_bruteplan` column,
  `project set-gameplan brute <name>`/`--clear`, shown in `project current`,
  and consulted by `brute plan/run` (fallback `quick`).
- Confirmed design: `crack` stays a separate **offline** pillar (hash files, no
  host); `bust`/`fuzz`/`brute` are the **online**, host-related pillars, and a
  `brute` job can link to a project host so discovery feeds credential attacks.
...
| feroxbuster flag-coverage audit -- first-class support for the important ferox options we don't expose yet | Partially done | Adapter passes `--url/--wordlist/--json/--output/--silent/--no-state/--extensions/--depth(or --no-recursion)/--status-codes/--rate-limit/--threads/--proxy/--headers` + raw `extra_args`. **DONE: (1) response filters** `--filter-status/-size/-words/-lines/-regex` -- now first-class Stage fields (usable in gameplan JSON), CLI flags on bust run/plan (apply to all stages), and part of the fingerprint (2026-09-24); **(3) auto-collection**: `--collect-extensions` now wired (the `collect_extensions` Stage field was a no-op before Extension Intelligence). **Still to do, by value:** **(2) `--extract-links` (-e)** parse response bodies for more URLs; rest of **(3)** `--collect-backups`/`--collect-words`; **(4) `--auto-tune`/`--auto-bail`** WAF/error backoff; **(5) `--redirects` (-r)**, **`--insecure` (-k)**; **(6) `--replay-proxy` (-P)** send only matches to Burp; **(7) auth**: `--cookies` (-b), first-class auth headers; **(8) non-GET**: `--methods`, `--data`, `--query`; **(9) runtime governance**: `--scan-limit`, `--time-limit`, `--timeout`; **(10) `--dont-scan`** path/regex exclusions. |
